...
const CareerForm: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<FormData> = data => {
    ...
    reset();
    localStorage.removeItem('careerFormValues');
  };

  watch(data => {
    localStorage.setItem('careerFormValues', JSON.stringify(data));
  });

  useEffect(() => {
    const savedCareerData = localStorage.getItem('careerFormValues');
    if (savedCareerData !== null) {
      const result = JSON.parse(savedCareerData);
  ...
  return (
    <div>
      <form
        ...
        className="flex flex-col gap-[16px] xl:gap-[36px] "
      >
        <div className="flex flex-col gap-[16px] md:flex-row md:gap-[20px] xl:gap-[24px]">
          <div className="flex flex-col gap-[16px] xl:gap-[24px] md:w-[221px]  xl:w-[283px] ">
            <div className="relative">
              <label
                className={`label-form  ${errors.fullName ? 'text-red-500' : 'text-white'} xl:mb-[4px] `}
              >
                Full name
              </label>
              <input
              ...
              />
              {errors.fullName && (
                <p className="error-message label-form  absolute right-0 bottom-[-24px] md:bottom-[-24px] xl:bottom-[-24px] text-red-500 ">
            ...
            <div className="relative">
              <label
                className={`label-form xl:mb-[4px] ${errors.email ? 'text-red-500' : 'text-white'}  `}
              >
                E-mail
              </label>
              <input
            ...
            <div>
              <label className="label-form xl:mb-[4px]  text-white">
                Position
              </label>
              <input
                type="text"
            ...
            <div className="relative">
              <label
                className={`label-form xl:mb-[4px] ${errors.phone ? 'text-red-500' : 'text-white'}  `}
              >
                Phone
          ...
          <div className="flex flex-col md:self-end md:mb-auto">
            ...
            <textarea
              {...register('formText')}
              className="input-form-text bg-white/5 h-[193px] w-full px-[8px]  resize-none md:h-[228px] md:w-[221px] xl:h-[268px]  xl:w-[292px]"
            />
          </div>
        </div>
